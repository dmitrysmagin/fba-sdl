...
static INT_PTR CALLBACK DefInpProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (Msg) {
		case WM_INITDIALOG: {
			...
			UINT idsString[5] = { IDS_SPATH_PREVIEW,IDS_SPATH_TITLES,IDS_SPATH_CHEATS,IDS_SPATH_HISCORE, IDS_SPATH_SAMPLES };
		...
		case WM_COMMAND: {
			...
			if (LOWORD(wParam) == IDOK) {
				...
				if (LOWORD(wParam) >= IDC_SUPPORTDIR_BR1 && LOWORD(wParam) <= IDC_SUPPORTDIR_BR5) {
