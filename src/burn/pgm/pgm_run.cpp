...
int pgmInit()
{
	...
	if (bSaveCRoms) {
		char name[128];
		sprintf (name, "%s_68kprog_decrypted.bin", BurnDrvGetTextA(DRV_NAME));
		FILE *fr = fopen(name, "wb");
		fwrite (PGM68KROM, nPGM68KROMLen, 1, fr);
		fclose (fr);
		if (nEnableArm7) {
			sprintf (name, "%s_arm7external_decrypted.bin", BurnDrvGetTextA(DRV_NAME));
			fr = fopen(name, "wb");
			fwrite (PGMUSER0, nPGMExternalARMLen, 1, fr);
			fclose (fr);
		}
		bSaveCRoms = false;
	}
